...
That is not the same as there being no data. The corpus places `3SPH` **3 times** and
`docs/gam_schema.md` harvests **11 properties** from those instances, with names,
types and value ranges; see its `3SPH` section. Which of them a parent registers
...
inherited set is not empty: 11 properties across 3 instances of `3SPH` are harvested
