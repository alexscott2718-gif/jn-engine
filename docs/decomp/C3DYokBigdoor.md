...
`C3DYokBigdoor` is a placeable **mechanisms moving parts** object (family `mechanisms_moving_parts`, wave 6). It walks the class vtable with 1 owned method; its `.gam`-driven parameters and assets are registered in `InitObject` and listed below.
...
|---|---|---|---|
| `vfunc_01_007` | `00449210` | InitObject (property + asset registration) | registers 4 `.gam` properties (`ItemClosed`, `Next`, `DoorSpeed`, `OpenTime`) |
...

{
  ...
  *(undefined1 *)&this[0x15d].vftable = 0;
  ...
  uVar2 = FUN_00477ba0(this[0x12a].vftable,3);
...
Validation: Ghidra `DumpClass.java C3DYokBigdoor` (owned methods decompiled); `.gam` properties and assets resolved from the `InitObject` registrar calls with strings read directly from `Neutron.exe`. `.gam` value ranges cross-referenced via `docs/gam_schema.md`. Behavioral prose is derived from the decompiled bodies above; not runtime-validated.
