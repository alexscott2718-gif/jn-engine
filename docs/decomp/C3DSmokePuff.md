...
| Vftable(s) | `004b544c, 004b545c, 004b58ac, 004b58c0` |
| Ctor(s) | factory/constructor installs the vftables and registers the class id (see `docs/_gam_classids.tsv`) |
| Dtor(s) | scalar deleting destructor `vfunc_02_002` at `00440b50` |
| Ledger row | `docs/decomp_ledger.csv` |

`C3DSmokePuff` is a placeable **effects triggers nav cameras sound** object (family `effects_triggers_nav_cameras_sound`, wave 8). It walks the class vtable with 1 owned method; its `.gam`-driven parameters and assets are registered in `InitObject` and listed below.
...
|---|---|---|---|
| `vfunc_02_002` | `00440b50` | scalar deleting destructor | destroys the `OMediaClassStreamer` subobject and frees the allocation |

### Decompiled owned methods

**`vfunc_02_002` @ `00440b50`** — scalar deleting destructor

```c
C3DSmokePuff * __thiscall C3DSmokePuff::vfunc_02_002(C3DSmokePuff *this)

{
  byte in_stack_00000004;
  
  FUN_00440b80();
  OMediaClassStreamer::~OMediaClassStreamer((OMediaClassStreamer *)(this + 0x17e));
...
Validation: Ghidra `DumpClass.java C3DSmokePuff` (owned methods decompiled); `.gam` properties and assets resolved from the `InitObject` registrar calls with strings read directly from `Neutron.exe`. `.gam` value ranges cross-referenced via `docs/gam_schema.md`. Behavioral prose is derived from the decompiled bodies above; not runtime-validated.
