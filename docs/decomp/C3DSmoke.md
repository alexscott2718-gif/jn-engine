...
That is not the same as there being no data. The corpus places `3SM1` **1 time** and
`docs/gam_schema.md` harvests **14 properties** from those instances, with names,
types and value ranges; see its `3SM1` section. Which of them a parent registers
...
inherited set is not empty: 14 properties across 1 instance of `3SM1` are harvested
