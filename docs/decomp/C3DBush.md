...
| Vftable(s) | `004935fc, 0049360c, 00493a5c, 00493a70` |
| Ctor(s) | factory/constructor installs the vftables and registers the class id (see `docs/_gam_classids.tsv`) |
| Dtor(s) | scalar deleting destructor `vfunc_02_002` at `00411500` |
| Ledger row | `docs/decomp_ledger.csv` |

`C3DBush` is a placeable **world props terrain** object (family `world_props_terrain`, wave 7). It walks the class vtable with 1 owned method; its `.gam`-driven parameters and assets are registered in `InitObject` and listed below.
...
|---|---|---|---|
| `vfunc_02_002` | `00411500` | scalar deleting destructor | destroys the `OMediaClassStreamer` subobject and frees the allocation |

### Decompiled owned methods

**`vfunc_02_002` @ `00411500`** — scalar deleting destructor

```c
C3DBush * __thiscall C3DBush::vfunc_02_002(C3DBush *this)
...
Validation: Ghidra `DumpClass.java C3DBush` (owned methods decompiled); `.gam` properties and assets resolved from the `InitObject` registrar calls with strings read directly from `Neutron.exe`. `.gam` value ranges cross-referenced via `docs/gam_schema.md`. Behavioral prose is derived from the decompiled bodies above; not runtime-validated.
