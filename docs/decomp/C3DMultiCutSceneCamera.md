...
That is not the same as there being no data. The corpus places `3MCA` **114 times** and
`docs/gam_schema.md` harvests **63 properties** from those instances, with names,
types and value ranges; see its `3MCA` section. Which of them a parent registers
...
inherited set is not empty: 63 properties across 114 instances of `3MCA` are harvested
