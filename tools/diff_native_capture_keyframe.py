...
def markdown_report(out, top_n=12):
    s = out["summary"]
    lines = [
        f"# Native vs Capture Keyframe {out['keyframe']} Diff",
        "",
        "Generated by `tools/diff_native_capture_keyframe.py`.",
        "",
        "## Summary",
        "",
        "```text",
        f"schema                         {out['schema']}",
        f"in-frustum rows                {s['in_frustum_meshes']}",
        f"capture drawcalls              {s['capture_drawcalls_total']}",
        f"capture-matched native rows    {s['matched']}",
        f"capture-only drawcalls         {s['capture_only_drawcalls']}",
    ]
    if s.get("solver_inliers") is not None:
        gate = "PASS" if s.get("solver_inlier_gate_pass") else "FAIL"
    ...
    if s.get("ground_in_alignment") is False:
        lines.extend([
            "`GROUND` is not in the current alignment `in_frustum_meshes` set,",
    ...
    missing = [
        r for r in out["rows"]
        if r["match"] == "native_missing_texture" and r["mesh"] not in SCHOOL_NAMES
    ]
    missing.sort(key=lambda r: -r["face_count"])
    lines.extend([
        "",
    ...
    mismatches = [
        r for r in out["rows"]
        if r["match"] == "texture_mismatch" and r["mesh"] not in SCHOOL_NAMES
    ]
    mismatches.sort(key=lambda r: -r["face_count"])
    lines.extend([
        "",
...
def main():
    ...
    ap.add_argument(
        "--md-out",
        default=None,
        help="Markdown report path. Default is --out with .md suffix.",
    )
    ap.add_argument(
    ...
    md_path.write_text(markdown_report(out) + "\n")
