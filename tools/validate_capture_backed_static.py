#!/usr/bin/env python3
"""Validate the static capture-backed Level 1 render path."""
...
def validate_startup_split(log: str) -> None:
    required = [
    ...
    for marker in required:
        if marker not in log:
            raise RuntimeError(f"static capture startup missing marker: {marker}")
    forbidden = [
        "placements_load:",
        "placements_loaded=",
    ...
    for marker in forbidden:
        if marker in log:
            raise RuntimeError(f"static capture startup loaded visual-only asset: {marker}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--screenshot",
        default=root / "build/capture_backed_static_validation.png",
        type=Path,
    )
    parser.add_argument("--max-mae", default=2.0, type=float)
    ...
    validate_startup_split(log)
    mae, rms = compare(reference, screenshot)
    print(f"reference: {reference.relative_to(root)}")
    print(f"static screenshot: {screenshot.relative_to(root)}  MAE {mae:.3f}  RMS {rms:.3f}")
    if mae > args.max_mae:
        raise RuntimeError(f"static MAE {mae:.3f} > {args.max_mae:.3f}")
    print("capture-backed static PASS")
    return 0
